    ...
    public class MapeamentoBdCsv
    {
        ...
        public bool PedidoCarregar()
        {
            ...
            while (csvArquivo.LerLinha())
            {
                ...
                //Verifica se pedido já foi inserido
                sql = @"
                        Select
                            codigo
                        from
                        ...
                        where
                            cod_pedido_pocket = (
                        select RESULT from SP_HEX2UUID('" + c["id_pedido"].Replace("-", "") + "'))";

                pedidoJaCadastrado = D.Bd.Inull(sql, D.Roteiro.Transacao);
                //Guardar o Guid do pedido já inserido e pula para o próximo pedido
                if (pedidoJaCadastrado != null)
                {
                    Roteiro.lstPedidoJaCadastradoAnteriomente.Add(new Guid(c["id_pedido"]));
                    continue;
                }
                p = new Dictionary<string, string>();
                p.Add("@IOPERACAO", "I");
                ...
                string observacao = c["observacao"];

                // Retira tudo que não for digito, letra ou espaço!
                Regex RemoveChars = new Regex("[^0-9^A-z^\\ ]");
                observacao = RemoveChars.Replace(observacao, "");
                p.Add("@IOBSERVACAO", observacao);
                p.Add("@IBDI", c["bdi"]);
        ...
        public bool ItemPedidoCarregar(){
            ...
            while (csvArquivo.LerLinha())
            {
                ...
                codPedido = new Guid(c["id_pedido"]);
                
                //Evita a reinserção de item que já tiveram o pedido cadastrado anteriomente
                if (Roteiro.lstPedidoJaCadastradoAnteriomente.Contains(codPedido))
                {
                    continue;
                }
