    ...
    public partial class FrmRotaProcessa : FormBase
    {
        ...
        public void UpdateView()
        {
            ...
            try
            {
                ...
                QueryBuilder.AppendLine("WHERE (cliente.id_funcionario = " + D.Funcionario.Id + ") AND");
            ...
            if (grdRota.DataSource != null)
            {
                ...
                if (status != 3) // Se o status da rota não for desabilitada
                {
                    if (!(source.Rows[i]["id_cliente_pocket"] is DBNull))
                        D.Cliente.Carregar((Guid)(source.Rows[i]["id_cliente_pocket"]), 0, Cliente.IdTipoEnum.IdPocket);
                    else
                        D.Cliente.Carregar(new Guid(), (Int32)(source.Rows[i]["id_cliente"]), Cliente.IdTipoEnum.IdStore);
