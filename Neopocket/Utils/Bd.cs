...
namespace Neopocket
{
    public class Bd
    {
        ...
        /// <summary>
        /// Cria um dicionário que armazena o campo de um índice
        /// </summary>
        /// <param name="table"></param>
        /// <param name="field"></param>
        /// <returns></returns>
        public Dictionary<string, int> FieldIndexesFill(string table)
        {
            string sql = 
            @"SELECT
                    column_name
            ...
            while (reader.Read())
            {
                dic.Add(reader[0].ToString().ToLower(), i);
                ++i;
            }
            cmd.Dispose();
            return dic;
        ...
        /// <summary>
        /// SqlCeResultSet scenario
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ImportaLinha(string tabela, KeyValuePair<string, string> dic)
        {
            int i=0, nTick,nTotalTicks;
        ...
        public Guid Guid(string qry, SqlCeTransaction dbTrans)
        {  
            SqlCeCommand cmd = null;
            SqlCeDataReader reader = null;
            Guid r = new Guid();
            ...
            {
                FE.Show(ex.Message + " sql: " + qry, "Erro executando Bd.Guid(string qry, SqlCeTransaction dbTrans)", ex.StackTrace);
            }
            finally
            {
                reader.Close();
